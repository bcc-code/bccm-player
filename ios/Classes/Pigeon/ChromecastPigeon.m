// Autogenerated from Pigeon (v10.1.6), do not edit directly.
// See also: https://pub.dev/packages/pigeon

#import "ChromecastPigeon.h"

#if TARGET_OS_OSX
#import <FlutterMacOS/FlutterMacOS.h>
#else
#import <Flutter/Flutter.h>
#endif

#if !__has_feature(objc_arc)
#error File requires ARC to be enabled.
#endif

static id GetNullableObjectAtIndex(NSArray *array, NSInteger key) {
  id result = array[key];
  return (result == [NSNull null]) ? nil : result;
}

@interface CastSessionUnavailableEvent ()
+ (CastSessionUnavailableEvent *)fromList:(NSArray *)list;
+ (nullable CastSessionUnavailableEvent *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end

@implementation CastSessionUnavailableEvent
+ (instancetype)makeWithPlaybackPositionMs:(nullable NSNumber *)playbackPositionMs {
  CastSessionUnavailableEvent* pigeonResult = [[CastSessionUnavailableEvent alloc] init];
  pigeonResult.playbackPositionMs = playbackPositionMs;
  return pigeonResult;
}
+ (CastSessionUnavailableEvent *)fromList:(NSArray *)list {
  CastSessionUnavailableEvent *pigeonResult = [[CastSessionUnavailableEvent alloc] init];
  pigeonResult.playbackPositionMs = GetNullableObjectAtIndex(list, 0);
  return pigeonResult;
}
+ (nullable CastSessionUnavailableEvent *)nullableFromList:(NSArray *)list {
  return (list) ? [CastSessionUnavailableEvent fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    (self.playbackPositionMs ?: [NSNull null]),
  ];
}
@end

@interface ChromecastPigeonCodecReader : FlutterStandardReader
@end
@implementation ChromecastPigeonCodecReader
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    case 128: 
      return [CastSessionUnavailableEvent fromList:[self readValue]];
    default:
      return [super readValueOfType:type];
  }
}
@end

@interface ChromecastPigeonCodecWriter : FlutterStandardWriter
@end
@implementation ChromecastPigeonCodecWriter
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[CastSessionUnavailableEvent class]]) {
    [self writeByte:128];
    [self writeValue:[value toList]];
  } else {
    [super writeValue:value];
  }
}
@end

@interface ChromecastPigeonCodecReaderWriter : FlutterStandardReaderWriter
@end
@implementation ChromecastPigeonCodecReaderWriter
- (FlutterStandardWriter *)writerWithData:(NSMutableData *)data {
  return [[ChromecastPigeonCodecWriter alloc] initWithData:data];
}
- (FlutterStandardReader *)readerWithData:(NSData *)data {
  return [[ChromecastPigeonCodecReader alloc] initWithData:data];
}
@end

NSObject<FlutterMessageCodec> *ChromecastPigeonGetCodec(void) {
  static FlutterStandardMessageCodec *sSharedObject = nil;
  static dispatch_once_t sPred = 0;
  dispatch_once(&sPred, ^{
    ChromecastPigeonCodecReaderWriter *readerWriter = [[ChromecastPigeonCodecReaderWriter alloc] init];
    sSharedObject = [FlutterStandardMessageCodec codecWithReaderWriter:readerWriter];
  });
  return sSharedObject;
}

@interface ChromecastPigeon ()
@property(nonatomic, strong) NSObject<FlutterBinaryMessenger> *binaryMessenger;
@end

@implementation ChromecastPigeon

- (instancetype)initWithBinaryMessenger:(NSObject<FlutterBinaryMessenger> *)binaryMessenger {
  self = [super init];
  if (self) {
    _binaryMessenger = binaryMessenger;
  }
  return self;
}
- (void)onSessionEnded:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionEnded"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionEnding:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionEnding"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionResumeFailed:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionResumeFailed"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionResumed:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionResumed"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionResuming:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionResuming"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionStartFailed:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionStartFailed"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionStarted:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionStarted"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionStarting:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionStarting"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onSessionSuspended:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onSessionSuspended"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onCastSessionAvailable:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onCastSessionAvailable"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:nil reply:^(id reply) {
    completion(nil);
  }];
}
- (void)onCastSessionUnavailable:(CastSessionUnavailableEvent *)arg_event completion:(void (^)(FlutterError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.bccm_player.ChromecastPigeon.onCastSessionUnavailable"
      binaryMessenger:self.binaryMessenger
      codec:ChromecastPigeonGetCodec()];
  [channel sendMessage:@[arg_event ?: [NSNull null]] reply:^(id reply) {
    completion(nil);
  }];
}
@end

