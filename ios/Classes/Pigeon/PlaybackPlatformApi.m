...
+ (PlayerStateSnapshot *)fromList:(NSArray<id> *)list {
  ...
  PlaybackStateBox *enumBox = GetNullableObjectAtIndex(list, 1);
  pigeonResult.playbackState = enumBox.value;
...
+ (ChromecastState *)fromList:(NSArray<id> *)list {
  ChromecastState *pigeonResult = [[ChromecastState alloc] init];
  CastConnectionStateBox *enumBox = GetNullableObjectAtIndex(list, 0);
  pigeonResult.connectionState = enumBox.value;
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[NpawConfig class]]) {
    ...
    [self writeByte:146];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[BufferModeBox class]]) {
  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSString *arg_playerId = GetNullableObjectAtIndex(args, 0);
        RepeatModeBox *enumBox = GetNullableObjectAtIndex(args, 1);
  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSString *arg_playerId = GetNullableObjectAtIndex(args, 0);
        TrackTypeBox *enumBox = GetNullableObjectAtIndex(args, 1);
        TrackType arg_type = enumBox.value;
