...
class PlaybackApiImpl(private val plugin: BccmPlayerPlugin) :
    ...
    override fun moveQueueItem(playerId: String, fromIndex: Long, toIndex: Long, result: PlaybackPlatformApi.VoidResult) {
        ...
        playerController.moveQueueItem(fromIndex.toInt(), toIndex.toInt())
        result.success()
    }

    override fun removeQueueItem(playerId: String, id: String, result: PlaybackPlatformApi.VoidResult) {
        val playerController = getController(playerId)
        if (playerController == null) {
            result.error(Error("Player with id $playerId does not exist."))
            return
        }
        playerController.removeQueueItem(id)
        result.success()
    }

    override fun clearQueue(playerId: String, result: PlaybackPlatformApi.VoidResult) {
        val playerController = getController(playerId)
        if (playerController == null) {
            result.error(Error("Player with id $playerId does not exist."))
            return
    ...
    override fun setCurrentQueueItem(playerId: String, id: String, result: PlaybackPlatformApi.VoidResult) {
        ...
        playerController.setCurrentQueueItem(id)
        result.success()
    }

    override fun getQueue(playerId: String, result: PlaybackPlatformApi.Result<PlaybackPlatformApi.MediaQueue>) {
        val playerController = getController(playerId)
        if (playerController == null) {
            result.error(Error("Player with id $playerId does not exist."))
            return
